...
[GNU's cp Man Page](https://man7.org/linux/man-pages/man1/cp.1.html)
...
| `-S`       | `--suffix=SUFFIX`          | override the usual backup suffix                                                     | done            |
...
| `-u`       |                            | equivalent to `--update[=older]`                                                     | done            |
|            | `--update[=UPDATE]`        | control which existing files are updated; UPDATE={all,none,none-fail,older(default)} | done            |
